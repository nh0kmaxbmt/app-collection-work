# 07 — Core Context Store & Persistence Layer

## Source
`docs/gemini-archive/component/gemini-07-Core Context Store & Persistence Layer.txt`
Data schema: `docs/gemini-archive/manual/gemini_manual-02-app-data.md`

## Feature Description
React Native Context state management engine with AsyncStorage persistence. Manages templates, active runs, and history logs. Seeds mock data on first launch. Exposes atomic actions for the full run lifecycle.

## Requirements
1. Create `FlightManualProvider` and custom hook `useFlightManual()`
2. Manage three state variables initialized from AsyncStorage: `templates`, `activeRun`, `historyLogs`
3. On first launch (empty storage), seed with the Gym Session Prep template and dummy run logs from the manual-02 blueprint
4. Atomic functions:
   - `startRun(templateId)` — instantiate RunInstance, record startedAt, save
   - `selectBranch(optionKey)` — inject conditional steps into currentSteps after the dependency check step
   - `toggleStep(stepId)` — mutate isCompleted; unlock dependent steps
   - `completeRun()` — calculate duration, append RunLog, clear activeRun to null
5. All async mutations wrapped in try/catch with defensive error logging

## Storage Keys (from manual-02)
- `flightmanual::templates`
- `flightmanual::active_run`
- `flightmanual::run_logs`

## Adaptation Notes (Expo / React Native)
- Requires `@react-native-async-storage/async-storage` (`npx expo install @react-native-async-storage/async-storage`)
- AsyncStorage is async — all reads/writes use `await`
- JSON.stringify/parse required (AsyncStorage only stores strings)
